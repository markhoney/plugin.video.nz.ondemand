...
class tv3:
 def __init__(self):
  ...
  self.urls['img-re'] = '\.ondemand\.tv3\.co\.nz/Portals/0/AM/'
 ...
 def _indexreal(provider): #Create a list of top level folders as scraped from TV3's website
  page = webpage("%s/tabid/56/default.aspx" % (base_url(provider)))
  if page.doc:
 ...
 def _dateduration(self, ad): #Search a tag for aired date and video duration
  if ad:
   ...
   if aired:
    ...
    info["Premiered"] = resources.tools.xbmcdate(aired.group(1))
 ...
 def _itemdiv(soup, provider, count): #Scrape items from a div-style HTML page
  baseurl = base_url(provider)
   ...
   if href:
    if link.string:
     ...
     if title <> "":
      ...
      info["FileName"] = "%s?ch=TV3&id=%s&info=%s" % (self.base, "%s,%s,%s,%s" % (href.group(1), href.group(2), href.group(3), href.group(4)), urllib.quote(str(info)))
      resources.tools.addlistitem(info, tv3_urls["Fanart"], 0)

 def _itemshow(soup, provider, count, title): #Scrape items from a show-style HTML page
  info = resources.tools.defaultinfo()
  info["Studio"] = provider
  bold = soup.find('b')
  if bold:
 ...
 def _itemtable(soup, provider, count, title): #Scrape items from a table-style HTML page
  info = resources.tools.defaultinfo()
  info["Studio"] = provider
  link = soup.find('a')
  if link:
 ...
 def _itematoz(soup, provider, count): #Scrape items from an AtoZ-style HTML page
  baseurl = base_url(provider)
  info = resources.tools.defaultinfo()
  info["Studio"] = provider
  if soup.find('h5'):
    ...
    if href:
     if link.string:
      ...
      if title <> "":
       ...
       #resources.tools.addlistitem(info, tv3_urls["Fanart"], 0)
    resources.tools.addlistitems(infoitems, tv3_urls["Fanart"], 0)
...
def INDEX_FOLDERS(): #Create a list of top level folders for the hierarchy view
 ...
 for index in folders:
  ...
  resources.tools.addlistitem(info, tv3_urls["Fanart"], 1, count)
...
def INDEX_FOLDER(folder): #Create second level folder for the hierarchy view, only showing items for the selected top level folder
 ...
 for index in infopages:
  if infopages[index][1] == folder:
   ...
   resources.tools.addlistitem(info, tv3_urls["Fanart"], 1)
 if folder == "Shows":
  INDEX_SHOWS("tv3")
 elif folder == "Search":
  INDEX_SEARCH()
...
def INDEX(provider): #Create a list of top level folders as scraped from TV3's website
 doc = resources.tools.gethtmlpage("%s/tabid/56/default.aspx" % (base_url(provider))) #Get our HTML page with a list of video categories
 if doc:
  a_tag = SoupStrainer('a')
...
def SHOW_EPISODES(catid, provider): #Show video items from a normal TV3 webpage
 doc = resources.tools.gethtmlpage("%s%s%s" % (base_url("tv3"), tv3_urls["CAT"], catid))
 if doc:
  a_tag=SoupStrainer('div')
  html_atag = BeautifulSoup(doc, parseOnlyThese = a_tag)
...
def SHOW_SEARCH(catid, searchterm): #Show video items from a normal TV3 webpage
 doc = resources.tools.gethtmlpage("%s/search/tabid/%s/Default.aspx?amq=%s" % (base_url("tv3"), catid, searchterm.replace(" ", "+")))
 if doc:
  a_tag=SoupStrainer('div')
  html_atag = BeautifulSoup(doc, parseOnlyThese = a_tag)
  programs = html_atag.findAll(attrs={"class": "results"})
 ...
 if doc:
  ...
  if videoid:
   ...
   if videoplayer:
    ...
    info["FileName"] = url
    resources.tools.addlistitem(info, tv3_urls["Fanart"], 0, 1, uri)
   else:
